...
def test_markdown_reporter_render_scan(sample_analysis_result: AnalysisResult):
    ...
    assert "# 🛡️ CodeSentinel Security & Architecture Audit Report" in output
    assert "Overall Health" in output
    assert "Summary Statistics" in output
    assert "| Category | Total | Critical | High | Medium | Low | Info |" in output
    assert "| Rule ID | Severity | Location | Message |" in output
    assert "Generated by CodeSentinel" in output
...
def test_markdown_reporter_render_comparison(sample_analysis_result: AnalysisResult):
    ...
    assert "# 🛡️ CodeSentinel Differential Analysis Report" in output
    assert "Regression Status" in output
    assert "Findings Transition Summary" in output
    assert "Unchanged Defects" in output
